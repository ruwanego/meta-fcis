  ...
  try {
    ...
    if (!routeConfig) {
      throw new RuntimeError(
        "ROUTE_NOT_FOUND",
        `Route not found for path or name: '${request.route}'`
      );
    }
    ...
    if (routeConfig.schema !== undefined) {
      try {
        ...
        throw new RuntimeError(
          ...
          `Request payload validation failed: ${err instanceof Error ? err.message : String(err)}`,
          err
        );
      }
    }
    ...
    if (routeConfig.dependencies) {
      try {
        ...
        throw new RuntimeError(
          ...
          `Failed to load dependencies: ${err instanceof Error ? err.message : String(err)}`,
          err
        );
      }
    }
    ...
    try {
      ...
      throw new RuntimeError(
        ...
        `Pure function invocation failed: ${err instanceof Error ? err.message : String(err)}`,
        err
      );
    }
    ...
    try {
      ...
      throw new RuntimeError(
        "CORE_OUTPUT_INVALID",
        `IntentSet validation failed: ${err instanceof Error ? err.message : String(err)}`,
        err
      );
    }
    ...
    throw new RuntimeError(
      "INTERNAL_ERROR",
      `An unexpected internal error occurred: ${err instanceof Error ? err.message : String(err)}`,
      err
    );
  }
}
