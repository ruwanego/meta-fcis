# Plugin: HTTP Transport Specification

## Purpose

Serve a shell runtime over HTTP. `@meta-fcis/plugin-transport-http` is the only package allowed to know HTTP: it maps incoming requests to core `Request`s, passes engine-decided error statuses through unchanged, and enforces route-declared HTTP methods. Built on `node:http` with no runtime dependency beyond `@meta-fcis/core`.
