# Plugin: Model Schema Specification

## Purpose

Model-driven request validation. `@meta-fcis/plugin-schema` provides a `SchemaAdapter` that interprets the graph's own `models` declarations — the graph is the schema language; no external validator, zero runtime dependencies beyond `@meta-fcis/core`. Failures surface through core as `REQUEST_VALIDATION_FAILED` (400).
